...
def docker_run_command(command)
    ...
    uid = `id -u`
    run_string = "docker run --rm #{docker_volume_string} -it #{PROJECT_NAME}-build:latest /bin/bash -c \"sudo usermod -u #{uid} #{DOCKER_USER} ; export BITBAKEDIR=sources/poky/bitbake;  #{command} \""
    sh run_string
end
